...
F=(H-l)*(A-C)-(k-l)*(m-C);E=(l-j)*(m-x)-(H-j)*(C-x);t=(k-j)*(m-x)-(H-j)*(A-x);return(G*F<=2e-10&&E*t<=2e-10)};if(!(s instanceof BMapGL.Polyline&&r instanceof BMapGL.Polygon)){console.error("鍙傛暟鍑洪敊,浼犲叆鍊奸潪鎶樼嚎鍜屽杈瑰舰");return false}s=s.getPath().map(function(j){return{"lng":j.lng,"lat":j.lat}});r=r.getPath().map(function(j){return{"lng":j.lng,lat:j.lat}});if(s.length<1||r.length<=2){console.error("鍙傛暟鍑洪敊,浼犲叆鍊奸潪鎶樼嚎鍜屽杈瑰舰");return false}var e=[],f=[];for(var q=0;q<s.length;q++){if(b.isPointInPolygon(s[q],r)){return true}}for(var h=1;h<s.length;h++){e.push([s[h-1],s[h]])}for(var p=1;p<r.length;p++){f.push([r[p-1],r[p]])}f.push([r[r.length-1],r[0]]);for(var o=0;o<e.length;o++){for(var i=0;i<f.length;i++){if(g(e[o][0],e[o][1],f[i][0],f[i][1])){return true}}}return false}})();