...
const disasterDataset =
  "http://10.23.56.29:8090/iserver/services/data-gis-2/rest/data";
const disasterDatasetName = "address_disaster@DataSourceDisasterGIS";
import { calcRatioNP } from "@/utils/number";
import earthquakeIcon from "@/assets/icons/earthquake-warning.gif";
import wainIcon from "@/assets/icons/icon-warning.gif";
// import { earthquakeData } from "../../data/earthquake-mock.js";
import { earthquakeData } from './data.js'
...
export default {
  data() {
    return {
      ...
      // 高风险弹窗1
      riskdialogVisible1: false,

      // 高风险弹窗2
      riskdialogVisible2: false,
...
// }).addTo(map);
      ...
      //   "http://10.23.56.29:8090/iserver/services/map-Precipitation/rest/maps/PrecipitationMap",
      //   {
      //     transparent: true,
      //     noWrap: true,
      //     //opacity: 0.6
      ...
      // var overlayLayer = new L.supermap.TiledMapLayer(
      //   "http://10.23.17.229:8090/iserver/services/map-gis-disaster--3--2/rest/maps/oneAndTwoLevel",
      //   {
      //     transparent: true,
      //     noWrap: true,
      //     //opacity: 0.6
      ...
      // const icon = L.icon(earthquakeIcon, [32, 32], -20);
      const earthQuakeIcon = L.icon({
        iconUrl: earthquakeIcon, // jpg png gif都可以
        iconSize: [70, 70],
      });

      //    序号	发震时刻(UTC+8)	经度(°)	纬度(°)	震源深度(千米)	震级(M)	震中位置
      // 1	2025-3-18 19:21:53	104.59	29.64	8	3.4	四川内江市威远县
      earthquakeData.map((item) => {
        const earthQuakeMarker = L.marker([item.latitude, item.longitude], {
          icon: earthQuakeIcon,
        }).addTo(map);

        earthQuakeMarker.on("click",  () =>{
          this.earthquakePoint = item
          
          setTimeout(() => {
            self.riskdialogVisible1 = true;
          }, 300)
        });
      });
      ...
      // const icon = L.icon({
      //   iconUrl: wainIcon, // jpg png gif都可以
      //   iconSize: [70, 70],
      // });
      // const marker = L.marker([31.561596, 103.524355], { icon: icon }).addTo(
      //   map
      // );
    ...
    loadLines() {
      ...
      const loadByPage = (
        ...
        const service = new SuperMap.GetFeaturesByBoundsService(
          disasterDataset
        );
    ...
    loadLinesByBounds(bounds) {
      ...
      var geometry = L.supermap.Util.transform(
        geo,
        L.CRS.EPSG4326,
        L.CRS.EPSG3857
      );
      ...
      let service = new L.supermap.QueryService(
        "http://10.23.17.229:8090/iserver/services/map-china400/rest/maps/China"
      );
      service.queryByGeometry(queryParams, function (serviceResult) {
        self.loading = false;
        var result = L.supermap.Util.transform(
    ...
    renderVisiblePoints() {
      ...
      const sqlParam = new SuperMap.GetFeaturesByBoundsParameters({
        ...
        bounds: new SuperMap.Bounds(sw.lng, sw.lat, ne.lng, ne.lat),
      ...
      L.supermap
        ...
        .getFeaturesByBounds(sqlParam, function (serviceResult) {
          ...
          features.forEach(function (feature) {
            ...
            var marker = L.marker([lon, lat], { icon: myIcon }).addTo(
              pointLayer
            );
    ...
    async getDisasterType() {
      ...
      {
        const sql = "ds_type = '崩塌'";
        const res = await this.queryDisasterDataset(
          disasterDataset,
          getSql(disasterDatasetName, sql)
        );
        this.disasterTypeModel.bt = res.result.totalCount || 0;
      }

      {
        const sql = "ds_type = '塌陷'";
        const res = await this.queryDisasterDataset(
          disasterDataset,
          getSql(disasterDatasetName, sql)
        );
    ...
    async getRectificationProgress() {
      ...
      {
        const sql =
          "treatment_situation = '已处治修复' or treatment_situation = '未处治修复' or treatment_situation = '正在处治修复'";
        const res = await this.queryDisasterDataset(
          disasterDataset,
          getSql(disasterDatasetName, sql)
        );
      ...
      {
        const sql =
          "treatment_situation = '正在处治修复' or treatment_situation = '未处治修复'";
        const res = await this.queryDisasterDataset(
          disasterDataset,
          getSql(disasterDatasetName, sql)
        );
